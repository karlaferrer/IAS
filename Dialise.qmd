...
plot2 <- arrange_ggsurvplots(splots, print = TRUE, ncol = 2, nrow = 4)
...
Os gráficos de Kaplan Meier ilustrando o efeito de cada covariável e respectivos resultados dos testes log-rank são mostrados na @fig-Figura2 . Verifica-se visualmente maior diferença no comportameto das curvas para a variável hipertensão,  indicando que a presença dessa comorbidade aumenta o tempo de sobrevivência dos pacientes. Diferenças significativas no teste foram observadas para as covariáveis DM, hipertensão, doença hepática e DPOC.
